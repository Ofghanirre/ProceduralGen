...
{
public:
    explicit ValueNoise(unsigned seed = 2016) : r(), permutationTable()
    {
...
Noise ValueNoiseGenerator::genNoise(uint seed, uint width, uint height, uint frequency) {
    ...
    for (unsigned j = 0; j < height; ++j) {
        for (unsigned i = 0; i < width; ++i) {
            ...
            noiseMap[j * width + i] = noise.eval(Vec2f<float>(i, j) * frequency);
        }
    }
