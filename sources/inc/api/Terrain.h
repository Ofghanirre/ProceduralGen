...
*/
struct TerrainNode {
    TerrainNode(INoiseGenerator& noiseGenerator, int (*mappingFunction)(int)): noiseGenerator(noiseGenerator), mappingFunction(mappingFunction) {};
...
public:
     ...
     */
    void addTerrainNode(INoiseGenerator& noiseGenerator, int (*mappingFunction)(int)) {
        m_terrainNodes.push_back(TerrainNode(noiseGenerator, mappingFunction));
    }
    ...
    void addTerrainNode(INoiseGenerator& noiseGenerator, NoisePunderationFunction eFunction) {
        ...
        switch(eFunction) {
            ...
            case (INVERSE):
                ...
                break;
            case (SQUARE):
                function = [](int value) { return value * value; };
                break;
            case (CUBE):
                function = [](int value) { return value * value * value; };
                break;
            case (ABS):
                function = [](int value) { return abs(value); };
                break;
            default:
        ...
        for (TerrainNode &terrainNode : m_terrainNodes) {
            ...
            for (int h = 0; h < height; ++h) {
                for (int w = 0; w < width; ++w) {
                    bitmap[std::pair<int, int>(w, h)] += terrainNode.mappingFunction(noise.getBitmap()[std::pair<int, int>(w, h)]);
                }
            }
        }
