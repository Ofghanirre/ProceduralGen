...
public:
    ...
    void addTerrainNode(INoiseGenerator& noiseGenerator, NoisePunderationFunction eFunction) {
        switch(eFunction) {
            case (IDENTITY): {
                    auto function = IdentityPunderationFunction();
                    return addTerrainNode(noiseGenerator, function);
            }
            case (INVERSE): {
                auto function = ProductPunderationFunction<int>(-1);
                return addTerrainNode(noiseGenerator, function);
            }
            case (SQUARE): {
                auto function = SquarePunderationFunction(2);
                return addTerrainNode(noiseGenerator, function);
            }
            case (CUBE): {
                auto function = SquarePunderationFunction(3);
                return addTerrainNode(noiseGenerator, function);
            }
            case (SQUARE_ROOT): {
                auto function = SquarePunderationFunction(0.5);
                return addTerrainNode(noiseGenerator, function);
            }
            case (ABS): {
                auto function = AbsPunderationFunction();
                return addTerrainNode(noiseGenerator, function);
            }
            default:
